...
const scrapeRecoPublicData = async (
  ...
  const data = (await (
    await fetch(makeUrl(category, skipItems))
  ).json()) as RecoPublicResponse;

  let productIds = data.Items.map((item) => item.Id);
  if (skipItems + NUMBER_OF_GAMES_PER_REQUEST < data.PagingInfo.TotalItems) {
